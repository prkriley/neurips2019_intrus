...
# __this__ notebook fine-tunes a transformer pre-trained with `pretrain_enru_8gpu.ipynb` samplng trajectories from model. It requires all the same data files plus three more files generated by the pretrain notebook:
# * __`enru_base_pretrain/inp.voc`__ : english vocabulary containing all bpe tokens
# * __`enru_base_pretrain/out.voc`__ : russian vocabulary containing all bpe tokens
# * __`enru_base_pretrain/checkpoint_final.npz`__ : russian vocabulary containing all bpe tokens
...
NUM_GPU=3
STEP_K=50
experiment_name = "enru_train_v1_{}k_{}gpu".format(STEP_K, NUM_GPU)
...
#inp_voc = Voc.load('./enru_base_pretrain/inp.voc')
#out_voc = Voc.load('./enru_base_pretrain/out.voc')
...
trainer = MultiGPUTrainer(
    'trainer', make_model, optimizer_opts=dict(base_lr=5e-4, warmup_time=4000)
)
...
load('enru_full_pretrain_v1_50k_3gpu/checkpoint_final.npz', tf.trainable_variables())
...
hypo_scores = decoder.translate_line('i am the monument to all your sins .',
                                     beam_size=4, beam_spread=50, verbose=True)
hypo_scores = decoder.apply_length_penalty(hypo_scores, len_alpha=1.0)
...
for t in trange(STEP_K*1000):
    batches = chain(*[next(train.parallel_batcher) for _ in range(2)])
